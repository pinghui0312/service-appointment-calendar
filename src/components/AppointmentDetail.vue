...
import { createAppointment, updateAppointment } from "../../amplify/backend/functions";
...
const onClickSubmit = () => {
    ...
    //     dateTime: dateTime.value.toLocaleString()
    // }
        ...
        <form :onsubmit="onClickSubmit">
            <div className="flex tab:flex-row flex-col flex-1 items-center gap-4 flex-wrap">
                <div className="flex flex-col gap-y-1 w-full">
                    ...
                    <input required className="border border-gray rounded-[8px] p-2" v-model="dentistName"
                        placeholder="Dentist Name" />
                </div>
                <div className="flex flex-col gap-y-1 tab:flex-[calc(50%-16px)] flex-1 w-full">
                    <p className="font-semibold">Equipment*</p>
                    <select required className="border border-gray rounded-[8px] p-2" v-model="equipment">
                ...
                <div className="flex flex-col gap-y-1 tab:flex-[calc(50%-16px)] flex-1 w-full">
                    ...
                    <DatePicker className="flex w-full border" v-model="dateTime" placeholder="dateTime" />
                </div>
                <div className="flex flex-col gap-y-1 w-full">
                    <p className="font-semibold">Notes</p>
                    <textarea className="border border-gray rounded-[8px] p-2" v-model="notes" placeholder="Notes" />
                </div>
            </div>
